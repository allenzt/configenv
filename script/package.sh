...
case "$PACKAGE_ARCH" in
    ...
    'arm32v7')
        CMAKE_ARGS="$CMAKE_ARGS -DCPACK_DEBIAN_PACKAGE_ARCHITECTURE=armel"
    ...
    debian*.arm32v7)
        SETUP_COMMAND=$'
            ...
            apt-get install -y --no-install-recommends \
                ...
                libcurl4-openssl-dev:armel libssl-dev:armel uuid-dev:armel &&
        ...
        CMAKE_ARGS="$CMAKE_ARGS -DCMAKE_C_COMPILER=arm-linux-gnueabi-gcc"
        CMAKE_ARGS="$CMAKE_ARGS -DCMAKE_CXX_COMPILER=arm-linux-gnueabi-g++"
        ;;
    ...
    *)
        case "$PACKAGE_ARCH" in
            amd64)
                ;;
            arm32v7)
                MAKE_FLAGS="'CARGOFLAGS=--target armv7-unknown-linux-gnueabi'"
                MAKE_FLAGS="$MAKE_FLAGS 'TARGET=target/armv7-unknown-linux-gnueabi/release'"
                MAKE_FLAGS="$MAKE_FLAGS 'DPKGFLAGS=-b -us -uc -i --host-arch armel'"
                ;;
            aarch64)
    ...
    *)
        find "$LIBIOTHSM_BUILD_DIR" -maxdepth 1 -name "$LIBIOTHSM_PACKAGE_NAME-*-Linux.deb" | while read -r f; do
            case "$PACKAGE_ARCH" in
                ...
                'arm32v7')
                    echo "Renaming package $(basename "$f") to $(basename "$LIBIOTHSM_BUILD_DIR/${LIBIOTHSM_PACKAGE_NAME}_${VERSION}-${REVISION}_armel.deb")"
                    mv -f "$f" "$LIBIOTHSM_BUILD_DIR/${LIBIOTHSM_PACKAGE_NAME}_${VERSION}-${REVISION}_armel.deb"
                    ;;
